...
class TokenRepositoryImpl:
    ...
    def init():
        ...
        TokenRepositoryImpl.scheme = "(id,token,user_id,ip)"
        TokenRepositoryImpl.schemeNoBrackets = "id,token,user_id,ip"
    ...
    def findTokenByUserIdAndIp(args):
        ...
        try:
            db = Database()
            response = db.query(f"select t.id, t.token, t.user_id, t.ip from tokens t inner join users u on u.id = t.user_id where t.ip = {ip} and t.user_id = {userId};")
            db.done()
    ...
    def findNewId(args):
        ...
        try:
            db = Database()
            response = db.query(f"select max(id) from tokens;")
            db.done()
        except Exception as e:
            Logger.fail("An error occurred while query request", str(e))
    ...
    def validateToken(args):
        ...
        try:
            db = Database()
            response = db.query(f"select case when exists (select * from tokens t where t.token = :userName) then cast(0 as bit) else cast(1 as bit) end;")
            db.done()
        except Exception as e:
            Logger.fail("An error occurred while query request", str(e))
